...
L.TileLayer.WebGLHeatMap = L.Class.extend({
	...
	_plot: function () {
		...
		if (dataLen) {
            for (var i = 0; i < dataLen; i++) {
				...
				var radiusProportion = dataVal.length == 3 ? 1.0 : dataVal[3];
                heatmap.addPoint(
                        Math.floor(point.x),
                        Math.floor(point.y),
                        this._scale(latlng, radiusProportion),
						dataVal[2]);
            }
            heatmap.update();
            heatmap.display();
	...
	_scale: function (latlng, radiusProportion) {
		...
		var lngRadius = (this.options.size * radiusProportion / 40075017) * 360 / Math.cos(L.LatLng.DEG_TO_RAD * latlng.lat);
